#!/usr/bin/env python3
"""
Moirai Time Series Model - Zero-Shot Inference & Evaluation

Self-contained script to evaluate Salesforce Moirai model on your data.
⚠️  NOTE: Moirai is a pre-trained zero-shot model for INFERENCE ONLY.
It does NOT support gradient-based training/fine-tuning.

This script demonstrates:
- Loading pre-trained Moirai model from HuggingFace
...
DEPENDENCIES:
  ...
  Moirai (required): pip install uni2ts
...
def main():
    """Main evaluation function for Moirai zero-shot inference."""
    
    # Load configuration
    config_path = Path(__file__).parent / "configs" / "moriai.yaml"
    cfg = OmegaConf.load(config_path)
    
    logging.info("="*80)
    logging.info("Moirai Zero-Shot Inference & Evaluation")
    logging.info("="*80)
    ...
    logging.info("Loading pre-trained Moirai model...")
    ...
    if torch.cuda.is_available() and world_size > 1:
        ...
        logging.info(f"Using GPU {local_rank}")
