...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=bool,
        required=False,
        default=False  # evaluate on pretrain test or not
    )
...
def main():
    ...
    rank, world_size, local_rank, config, output_dir = setup(config_path=args.config_path, mode=TaskType.EVALUATE)
    # load config
    ...
    with DistributedGroupManager(backend=training_config.ddp_backend, rank=rank, local_rank=local_rank, world_size=world_size) as group_manager:
        ...
        logging.info(f"Evaluation dataset size: {len(eval_dataset)} samples")
        ...
        try:
            ...
            if rank == 0:
                final_results_save_path = os.path.join(training_config.output_dir, 'final_results.pkl')
                logging.info(f'Results saved to {final_results_save_path}')
                torch.save(results, final_results_save_path)
            ...
            logging.error(f"Evaluation failed with error: {e}, peform cleaning ...")
            raise
