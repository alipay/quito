...
class BaseModel(nn.Module, ABC):
    ...
    @abstractmethod
    def loss(self, x, y, x_mark=None, y_mark=None, **kwargs):
        """
    ...
    @abstractmethod
    def predict(self, x, y, x_mark=None, y_mark=None, **kwargs):
        """
...
class StatisticalModel(BaseModel):
    """
    This is the base for all Stats model
    """
    def forward(self, x: torch.Tensor, y: torch.Tensor = None, x_mark: torch.Tensor = None, y_mark: torch.Tensor = None, **kwargs):
        x = x.cpu().numpy()
        y = y.cpu().numpy() if y is not None else None
        predictions = self._forward(x, y, x_mark=None, y_mark=None, **kwargs)
        # convert to tensor
    ...
    def _forward(self, x: np.ndarray, y: np.ndarray = None, x_mark: np.ndarray = None, y_mark: np.ndarray = None, **kwargs) -> np.ndarray:
        raise NotImplementedError
